...
7.  **Final Processing and Output Generation:**
    *   `link-identify.zsh`: Combines linked markdown files from `results-markdown` with technical image information from `results-identify`. Creates final comprehensive markdown files in `results-mark-id` that include the original content plus technical metadata in preformatted code blocks. Handles cases where identify data is not available gracefully.
...
*   `results-mark-id/`: Final comprehensive markdown files created by `link-identify.zsh` that combine linked markdown content with technical image information from `results-identify`.
...
9.  Run `link-identify.zsh` to combine markdown files with technical image information into final comprehensive files.
10. (Optional) Run `run-id-batch.zsh` to gather metadata for asset images.
...
6. **Generate Final Comprehensive Files**
   ```bash
   # Combine markdown files with technical image information
   ./link-identify.zsh
   ```
...
echo "Identify results: $(ls results-identify/*.txt | wc -l)"
echo "Comprehensive final files: $(ls results-mark-id/*.md | wc -l)"
...
- Technical image metadata extracted to `results-identify/`
- Comprehensive final files in `results-mark-id/` combining markdown content with technical information
